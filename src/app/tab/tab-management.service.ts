...
export class TabManagementService {
  ...
  // pass data from hint root file to new hint tab
  ...
  // pass data from unity entry point to new simulation tab
  ...
  // input is the iframe generated by plotly
  // store content of plotly tabs in a special hidden folder (see "tabinfo" in config) --> next time the website is opened we can restore the tab 
  // by loading this file and converting it to an iframe again
  openPlotly(htmlContent: Uint8Array): Observable<never> {
    const path = `${uuidv4()}.plotly`;

    // since this method is called during python runtime, our changes would be overwritten by the fs sync after python execution if we wouldn't wait 
      ...
      switchMap(() => this.conf.getConfigOfCurrentExperience().pipe(
        switchMap(conf => concat(
          ...
          defer(() => {
            ...
            this._openTab.next(x); // this will cause config to be updated with the new tab
          })
        ))
      ))
    )
